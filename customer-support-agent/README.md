...
An AI agent that reads your customer support inbox and produces two kinds of output: replies that are ready to send (because the customer's question matches an entry in your FAQ) and drafts for your review (because it doesn't).
...
For each customer email in a batch:
...
Or download the repo as a zip from the [GitHub page](https://github.com/river-labs-inc/agents) (green **Code** button > **Download ZIP**) and unzip locally. You only need the files inside `customer-support-agent/`.
...
The three files in [`memory/`](./memory/) tell the agent who you are, how you want customer replies to sound, and which questions to handle automatically.
...
- [`memory/faq.md`](./memory/faq.md): customer Q&A pairs that gate auto-replies
...
- Paste the customer emails you want processed directly into the chat
...
Every time you find yourself editing a draft to give the same answer twice, that's a candidate for the FAQ. Open `memory/faq.md`, add a new Q&A pair, re-upload to Project Knowledge. The next scan will start handling that customer question as a ready-to-send reply.
...
customer-support-agent/
  ...
  memory/
    ...
    faq.md                        customer Q&A pairs that gate auto-replies
  prompts/
...
[Get started with River](https://rivereditor.com/customer-support-agent). Free to start, 5-minute setup.
